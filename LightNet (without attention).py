"""
LightNet (w/o Attention) for DQMARS Framework
...
def allocate_resource_slice(X):
    slice_ids = np.zeros(len(X))  # Assign all flows to slice 0
    return slice_ids

# -------------------------------
# Phase 2: QoS Profile Assignment (LightNet w/o Attention)
# -------------------------------
def assign_qos_profile(X, y, params):
    ...
    # Initialize TabNet with attention disabled
    model = TabNetClassifier(
        n_d=params.get("n_d", 8),
        n_a=0,  # Attention disabled
...
# -------------------------------
def main():
    parser = argparse.ArgumentParser(description="Train LightNet w/o Attention on DQMARS datasets")
    parser.add_argument("--dataset", type=str, required=True, help="Path to dataset CSV file")
    parser.add_argument("--config", type=str, default="./config/lightnet_wo_attention.json", help="Path to JSON config file")
    args = parser.parse_args()

    df = pd.read_csv(args.dataset)
    feature_cols = [col for col in df.columns if col not in ['Channel']]
    X = df[feature_cols].values
    y = df['Channel'].values - 1
    ...
    print("\n=== LightNet w/o Attention Performance Summary ===")
